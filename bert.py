...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        attention_scores = (
            torch.matmul(query, key.transpose(-2, -1)) + attention_mask
        ) / (d_k**0.5)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        attention_output = self.attention_dense(hidden_states, attention_mask)
        # add-norm layer
        attention_addnorm = self.add_norm(
            input=hidden_states,
        ...
        # feed forward
        interm_output = self.interm_dense(attention_addnorm)
        # output = self.out_dense(interm_output)
        # another add-norm layer
        output = self.add_norm(
            input=attention_addnorm,
...
class BertModel(BertPreTrainedModel):
    ...
    def embed(self, input_ids):
        ...
        # todo
        inputs_embeds = self.word_embedding(input_ids=input_ids) 

        # get position index and position embedding from self.pos_embedding
        pos_ids = self.position_ids[:, :seq_length]
        pos_embeds = self.pos_embedding(input_ids)
